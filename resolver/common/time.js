exports._getZeroTime = (time) => {
  const step = 1000 * 60 * 60 * 24;
  return Math.floor(time / step) * step;
};

exports._toSQLTime = (date) => {
  return date.toISOString().slice(0, 19).replace("T", " ");
};

exports._getCurrentDate = (date = null) => {
  const result = date ? new Date(date) : new Date();
  const parisDate = new Date(result.toLocaleString("en-US", { timeZone: "Europe/Paris" }));
  return parisDate;
};

exports._getNextNMonthDate = (date, n) => {
  const result = new Date(date.getFullYear(), date.getMonth() + n, date.getDate());
  const parisDate = new Date(result.toLocaleString("en-US", { timeZone: "Europe/Paris" }));
  return parisDate;
};

exports._toIsoString = (date) => {
  var tzo = -date.getTimezoneOffset(),
    dif = tzo >= 0 ? "+" : "-",
    pad = function (num) {
      return (num < 10 ? "0" : "") + num;
    };

  return date.getFullYear() + "-" + pad(date.getMonth() + 1) + "-" + pad(date.getDate()) + "T" + pad(date.getHours()) + ":" + pad(date.getMinutes()) + ":" + pad(date.getSeconds()) + dif + pad(Math.floor(Math.abs(tzo) / 60)) + ":" + pad(Math.abs(tzo) % 60);
};
